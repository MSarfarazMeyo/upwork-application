...
const { logger, focusAndTypeHandle, takeScreenshot, waitForComponentsToLoad, focusAndType } = require('./tools');
...
const setProjectDuration = async (page) => {
    ...
    await page.waitForSelector('.fe-proposal-job-estimated-duration [role="combobox"], .fe-proposal-job-estimated-duration button', { visible: true, timeout: 5000 }).then(
        (button) => {
            button.click()
    ...
    await page.waitForSelector('.fe-proposal-job-estimated-duration ul li:last-child').then(
        (li) => {
            li.click()
...
const setSriRate = async (page) => {
    ...
    await page.waitForSelector('.sri-form-card [role="combobox"]', { visible: true, timeout: 5000 }).then(
        div => div.click()
    )
    await Apify.utils.sleep(800);
    await page.waitForSelector('.sri-form-card ul li:nth-child(1)').then(
        li => li.click()
    )
};
...
const submitApplication = async (page) => {
    await Promise.race([
        page.waitForSelector('footer .air3-btn-primary').then(
            async button => {
    ...
    await page.waitForSelector('label[data-test=understand-and-agree] input', { timeout: 5000, visible: true }).then(
        async checkbox => {
            await checkbox.click()
            await page.click('.fe-proposal-fixed-price-confirmation-dialog .air3-btn-primary')
        }
    ).catch(err => {
        logger.info(err)
    });
...
exports.handleRefillConnects = async (page, autoRefillAmount) => {
    await page.waitForSelector('#connects-dropdown .up-btn,[aria-labelledby*="selectConnectsNumber"]')
    let chosed = false
    do {
        await Apify.utils.sleep(1000);
        await page.click('#connects-dropdown .up-btn,[aria-labelledby*="selectConnectsNumber"]')
        await page.waitForXPath(`//li/span/span[contains(text(), '${autoRefillAmount} for')]/parent::span/parent::li`, { visible: true, timeout: 1000 }).then(
            li => {
                chosed = true
                return li.click()
            }
    ...
    await page.waitForSelector('[data-qa="fulfill"]').then(button => button.click())
 ...
 */
exports.handleApplication = async ({ request, page, session, crawler }, { coverLetter, attachments = [], defaultAnswer, testMode, agency, freelancer, autoRefill, autoRefillAmount = '100', ignoreDuplicateProposals }) => {

    if (request.url.includes('connects')) {
        logger.info('Fill connects');
        await Promise.all([
            page.waitForXPath("//button[contains(text(), ' Buy Connects ')]").then(
                button => button.click()
            ),
   ...
   //     await page.waitForSelector('.fe-proposal-more-connects-needed-dialog .up-modal-close', { visible: true, timeout: 2000 }).then(
       //     button => button.click()
        ...
        await page.waitForSelector('.nav-messages ~ .nav-dropdown-account .nav-item-label', { visible: true }).then(
            button => button.evaluate(button => button.click())
        )
        await page.waitForXPath(
            ...
            async div => {
                const isSelected = await div.evaluate(div => div.closest('li').classList.contains('active'))
                if (!isSelected) {
        ...
        await page.waitForSelector('.up-fe-team-selector [role="combobox"]', { visible: true })
        await Apify.utils.sleep(3000);
        await page.click('.up-fe-team-selector [role="combobox"]')
        await Apify.utils.sleep(1000);
        await page.waitForXPath(`//div[contains(@class, "up-fe-team-selector")]//ul/li//span[contains(text(), "${agency}")]//ancestor::li`).then(
            li => li.click()
        )
        await page.waitForSelector('.up-fe-contractor-selector [role="combobox"]:not(.is-disabled)', { visible: true })
        await Apify.utils.sleep(3000);
        await page.click('.up-fe-contractor-selector [role="combobox"]')
        await Apify.utils.sleep(1000);
        if (!freelancer)
            freelancer = await page.evaluate(() => document.querySelector('.nav-messages ~ .nav-dropdown-account ul li a div div').textContent.trim())
        await page.waitForXPath(`//div[contains(@class, "up-fe-contractor-selector")]//ul/li//span[contains(text(), "${freelancer}")]//ancestor::li`, { timeout: 5000, visible: true }).then(
            li => li.click()
        ).catch(
            error => {
                logger.error(error);
  ...
  //  await page.waitForSelector('.fe-proposal-more-connects-needed-dialog .desktop-size-controls button', { visible: true, timeout: 5000 }).then(
   //     async button => {
    ...
    if (autoRefill) {
        await Promise.race([
            page.waitForXPath("//div[contains(text(), 'When you submit this proposal')]/strong").then(
                strong => strong.evaluate(strong => strong.textContent).then(
                    connects => parseInt(connects)
                )
            ),
            page.waitForXPath("//ng-pluralize[contains(text(), 'Connects')]/parent::span").then(
                strong => strong.evaluate(strong => strong.textContent).then(
                    connects => parseInt(connects.match(/\d+/))
                )
            ),
        ]).then(
            connects => {
    ...
    if (!testMode) {
        ...
        await Promise.race([
            page.waitForSelector('.form-error', { visible: true, timeout: 10000 }).then(
                async () => {
                    throw new Error(`${errors.length} validation errors found`);
                }
            ),
            page.waitForSelector('label[data-test=understand-and-agree]'/*'input[name=checkbox]'*/).then(
                async label => {
                    await label.click()
                    await Apify.utils.sleep(1000);
                    await page.waitForSelector('.air3-modal-footer .air3-btn-primary').then(
                        button => button.click()
                    )
                }
            ),
            page.waitForSelector('.up-alert.up-alert-danger', { visible: true, timeout: 10000 }).then(
                async alert => {
                    const error = await alert.evaluate(div => div.textContent)
                    throw new Error(error);
                }
            ),
            page.waitForSelector('.air3-alert.air3-alert-positive', { visible: true, timeout: 10000 }).then(
                async alert => {
            ...
            async error => {
                ...
                if (ignoreDuplicateProposals && error.message.includes('The user already has a pending proposal for this job')) {
 ...
 */
exports.handleLogin = async ({ request, page, session }, { username, password, securityQuestion }) => {
    ...
    try {
        ...
        await page.waitForSelector('#login_answer', { timeout: 10000 }).then(
            async () => {
