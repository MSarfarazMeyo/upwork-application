...
exports.focusAndType = async (page, selector, value) => {
  if (!selector || !value) {
    return;
  }
  ...
  await page.type(selector, value, {
    get delay() {
      return 5 * (Math.floor(Math.random() * 5) + 5);
    },
  });
  await utils.sleep(500);
};

exports.waitForComponentsToLoad = (page) =>
...
exports.takeScreenshot = async ({ request, page, number }) => {
  ...
  const key =
    `${Date.now()}_${number}_` +
    crypto.createHash("md5").update(`${Date.now()}`).digest("hex");
